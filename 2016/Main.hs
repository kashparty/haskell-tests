...
getAttribute att (Element _ atts _)
  = fromMaybe "" (lookup att atts)
getAttribute _ _
  = ""

getChildren :: String -> XML -> [XML]
getChildren name (Element n as (el@(Element name' _ _) : els'))
  | name == name' =  el : getChildren name (Element n as els')
getChildren name (Element n as (_ : els'))
  = getChildren name (Element n as els')
getChildren _ _
  = []

getChild :: String -> XML -> XML
getChild
  = ((head . (++ [Text ""])) .) . getChildren

addChild :: XML -> XML -> XML
-- Pre: the second argument is an Element
addChild el (Element n as els)
  = Element n as (els ++ [el])
...
getValue (Element _ _ es)
  = Text (concatMap ((\(Text s) -> s) . getValue) es)
...
parseAttributes s
  | firstS == '>' = ([], restS)
  | otherwise     = ((name, value) : atts, finalRest)
  where
    s'@(firstS : restS) = skipSpace s
    (name, rest) = parseName s'
    ('=' : rest') = skipSpace rest
    ('"' : rest'') = skipSpace rest'
    (value, '"' : rest''') = break (== '"') rest''
    (atts, finalRest) = parseAttributes rest'''
...
parse' "" (Element "" _ (res : _) : _)
  = res
parse' ('<' : '/' : rest) st
  = parse' rest' (popAndAdd st)
  where
    (_, '>' : rest') = break (== '>') rest
parse' ('<' : rest) st
  = parse' rest'' (Element name atts [] : st)
  where
...
expandXSL' ctx@(Element _ _ children) el@(Element "value-of" _ _)
  | null first        = [head children]
  | head first == '@' = [Text (getAttribute (tail first) ctx)]
  | null rest'        = [getValue (getChild first ctx)]
  | null res          = [Text ""]
  | otherwise         = [head res]
  where
    path = getAttribute "select" el
    (first, rest) = break (== '/') (dropWhile (`elem` "/.") path)
    rest' = dropWhile (`elem` "/.") rest
    recurse = (`expandXSL'` Element "value-of" [("select", rest')] [])
    res = concatMap recurse (getChildren first ctx)
expandXSL' ctx el@(Element "for-each" _ children)
  | null rest' = concatMap expandCtx newCtxs
  | otherwise  = concatMap recurse newCtxs
  where
    path = getAttribute "select" el
    (first, rest) = break (== '/') (dropWhile (`elem` "/.") path)
    rest' = dropWhile (`elem` "/.") rest
    recurse = (`expandXSL'` Element "for-each" [("select", rest')] children)
    newCtxs = getChildren first ctx
    expandCtx newCtx = concatMap (expandXSL' newCtx) children
