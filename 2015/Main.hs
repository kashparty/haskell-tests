...
assignArray (A xs) (I i) (I v)
  = A ((i, v) : [x | x@(i', _) <- xs, i' /= i])

updateVar :: (Id, Value) -> State -> State
updateVar (id, v) s
  | id `notElem` map fst s = (id, (Local, v)) : s
  | otherwise              = (id, (scope, v)) : delete (id, bnd) s
  where
    bnd@(scope, _) = lookUp id s
...
applyOp Index (A xs) (I i)
  | i `elem` map fst xs = I (lookUp i xs)
  | otherwise           = I 0
...
executeStatement (AssignA id i v) fs ps s
  = updateVar (id, newArr) s
  where
    newArr = assignArray (snd (lookUp id s)) (eval i fs s) (eval v fs s)
executeStatement (If e b b') fs ps s
  | eval e fs s == I 1 = executeBlock b fs ps s
  | otherwise          = executeBlock b' fs ps s
executeStatement st@(While e b) fs ps s
  | eval e fs s == I 1 = executeStatement st fs ps (executeBlock b fs ps s)
  | otherwise          = s
executeStatement (Call v p es) fs ps s
  | v == ""   = globs 
  | otherwise = updateVar (v, resV) (globs ++ getLocals s)
  where
    (as, b) = lookUp p ps
    vs = evalArgs es fs s
    s' = bindArgs as vs ++ getGlobals s
    s'' = executeBlock b fs ps s'
    globs = getGlobals s''
    ((_, (_, resV)) : _) = filter ((== "$res") . fst) (getLocals s'')
executeStatement (Return e) fs ps s
  = ("$res", (Local, eval e fs s)) : s

executeBlock :: Block -> [FunDef] -> [ProcDef] -> State -> State
-- Pre: All code blocks and associated statements are well formed
executeBlock ss fs ps s
  = foldl (\s st -> executeStatement st fs ps s) s ss
...
translate' (Const c) pMap ids
  = ([], Const c, ids)
translate' (Var id) pMap ids
  = ([], Var id, ids)
translate' (OpApp op e1 e2) pMap ids
  = (b1 ++ b2, OpApp op e1' e2', ids'')
  where
    (b1, e1', ids') = translate' e1 pMap ids
    (b2, e2', ids'') = translate' e2 pMap ids'
translate' (Cond p e1 e2) pMap ids
  | null (bp ++ b1 ++ b2) = ([], Cond p e1 e2, ids)
  | otherwise             = (bp ++ [If p' b1' b2'], Var id, ids''')
  where
    (bp, p', ids') = translate' p pMap ids
    (b1, e1', ids'') = translate' e1 pMap ids'
    (b2, e2', id : ids''') = translate' e2 pMap ids''
    b1' = b1 ++ [Assign id e1']
    b2' = b2 ++ [Assign id e2']
translate' (FunApp f es) pMap ids
  | f `elem` map fst pMap = (bs' ++ [Call id (lookUp f pMap) es], Var id, ids'')
  | otherwise             = (bs', FunApp f es, ids'')
  where
    (bs', id : ids'') = foldl accBlock ([], ids) es
    
    accBlock :: (Block, [Id]) -> Exp -> (Block, [Id])
    accBlock (bs, ids) e
      = (b ++ bs, ids')
      where
        (b, e', ids') = translate' e pMap ids
