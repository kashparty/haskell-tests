...
execStatement (If p b b') s
  | eval p s == 1 = execBlock b s
...
sub id x (Var id')
  | id == id' = Const x
  | otherwise = Var id'
sub id x (Apply op e e')
  = foldConst (Apply op (sub id x e) (sub id x e'))
sub id x (Phi e e')
  = foldConst (Phi (sub id x e) (sub id x e'))
...
scan fn (Assign id (Const c) : bs)
  | id == "$return" = (recFn, Assign id (Const c) : recB)
  | otherwise       = recs
  where
    fn' = sub id c . fn
    recs@(recFn, recB) = scan fn' bs
scan fn (Assign id e : bs)
  = (recFn, Assign id e : recB)
  where
    (recFn, recB) = scan fn bs
scan fn (If p b1 b2 : bs)
  = (recFn, If p b1' b2' : recB)
  where
    (fn', b1') = scan fn b1
    (fn'', b2') = scan fn' b2
    (recFn, recB) = scan fn'' bs
scan fn (DoWhile b c : bs)
  = (recFn, DoWhile b' c : recB)
  where
    (fn', b') = scan fn b
    (recFn, recB) = scan fn' bs
scan fn []
...
propagateConstants b
  | b == subB'   = subB'
  | otherwise    = propagateConstants subB'
  where
    (fn, b') = scan id b
    subB' = applySubs b'

    applySubs :: Block -> Block
    applySubs (Assign id e : bs)
      = Assign id (fn e) : applySubs bs
    applySubs (If p b1 b2 : bs)
      = If (fn p) (applySubs b1) (applySubs b2) : applySubs bs
    applySubs (DoWhile b c : bs)
      = DoWhile (applySubs b) (fn c) : applySubs bs
    applySubs []
      = []
...
unPhi :: Block -> Block
-- Pre: the block is in SSA form
unPhi (If p b1 b2 : (Assign v (Phi e1 e2)) : rest)
  = unPhi (
      If p (unPhi b1 ++ [Assign v e1]) 
           (unPhi b2 ++ [Assign v e2]) 
      : unPhi rest)
unPhi (If p b1 b2 : rest)
  = If p (unPhi b1) (unPhi b2) : unPhi rest
unPhi (DoWhile (Assign v (Phi e1 e2) : rest) p : rest')
  = Assign v e1 : unPhi (DoWhile (unPhi rest ++ [Assign v e2]) p : unPhi rest')
unPhi (DoWhile b p : rest)
  = DoWhile (unPhi b) p : unPhi rest
unPhi (b : bs)
  = b : unPhi bs
unPhi []
  = []
